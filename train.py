...
def gen_input_exact(img, skg, seg, xcenter, ycenter, patch_size=64, num_patch=1):
    # generate input skg with random patch from img
    # input img,skg [bsx3xwxh], xcenter,ycenter, size
    # output bsx5xwxh
...
def train(model, train_loader, val_loader, input_stack, target_img, target_texture,
    ...
    for i, data in enumerate(train_loader):
        ...
        if i % args.visualize_every == 0:
            pass
            # by J visualize_training(netG, val_loader, input_stack, target_img,target_texture, segment, vis, loss_graph, args)
